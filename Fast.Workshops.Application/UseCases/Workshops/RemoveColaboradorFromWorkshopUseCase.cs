...
namespace Fast.Workshops.Application.UseCases.Workshops
{
    public class RemoveColaboradorFromWorkshopUseCase
    {
        ...
        public async Task Execute(Guid workshopId, Guid colaboradorId)
        {
            ...
            if (existing is null)
                throw new NotFoundException("Colaborador não está registrado nesse workshop!");

            await _workshopColaboradorRepository.RemoveAsync(workshopId, colaboradorId);
        }
    }
}